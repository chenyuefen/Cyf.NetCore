...
using TM.Core.Logs.Abstractions;
using TM.Core.Logs.Extensions;
using TM.Core.Sessions;
using TM.Infrastructure.Extensions.Common;

namespace TM.Core.Logs.Core
{
    /// <summary>
    /// 日志操作抽象基类
    /// </summary>
    ...
    public abstract class LogBase<TContent> : ILog where TContent : class, ILogContent
    {
        ...
        /// <param name="context">日志上下文</param>
        /// <param name="session">用户会话</param>
        protected LogBase(string name, ILogProvider provider, ILogContext context)
        {
            Provider = provider;
            Context = context;
            Name = name;
        ...
        protected virtual void Execute(LogLevel level, ref TContent content)
        {
            ...
            try
            {
                content.Level = TM.Infrastructure.Helpers.Enum.GetName<LogLevel>(level);
                Init(content);
