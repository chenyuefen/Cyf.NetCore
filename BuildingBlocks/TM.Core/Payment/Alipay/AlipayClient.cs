...
using TM.Core.Payment.Alipay.Request;
...
namespace TM.Core.Payment.Alipay
{
    ...
    public class AlipayClient : IAlipayClient
    {
        ...
        public void SetTimeout(int timeout)
        {
            ...
            {
                logger?.LogError($"{DateTime.Now} 支付宝SetTimeout报错", ex.Message);
            }
        }
        ...
        public async Task<T> PageExecuteAsync<T>(IAlipayRequest<T> request, string accessToken, string reqMethod) where T : AlipayResponse
        {
            try
            {
                logger?.LogDebug($"{DateTime.Now} 支付宝PageExecuteAsync");
                ...
                else
                {
                    ...
                    if (reqMethod.ToUpper() == "GET")
                    {
                        ...
                        body = tmpUrl;
                        logger?.LogDebug(0, "Request Url:{body}", body);
                    }
                    else
                    {
                        //输出post表单
                        body = BuildHtmlRequest(txtParams, reqMethod);
                        logger?.LogDebug(0, "Request Html:{body}", body);
                    }
                }
            ...
            {
                logger?.LogError($"{DateTime.Now} 支付宝PageExecuteAsync报错", ex.Message);
                return null;
            }
        }
        ...
        public async Task<T> ExecuteAsync<T>(IAlipayRequest<T> request, string accessToken, string appAuthToken) where T : AlipayResponse
        {
            try
            {
                logger?.LogDebug($"{DateTime.Now} 支付宝ExecuteAsync");
                ...
                logger?.LogDebug($"{DateTime.Now} 支付宝Request:{query}");
                ...
                logger?.LogDebug($"{DateTime.Now} 支付宝Response:{body}");
            ...
            {
                logger?.LogError($"{DateTime.Now} 支付宝ExecuteAsync报错", ex.Message);
                return null;
            }
        }
        ...
        public string BuildHtmlRequest(IDictionary<string, string> sParaTemp, string strMethod)
        {
            try
            {
                logger?.LogDebug($"{DateTime.Now} 支付宝BuildHtmlRequest");
            ...
            {
                logger?.LogError($"{DateTime.Now} 支付宝BuildHtmlRequest报错", ex.Message);
                return null;
            }
        }
        #endregion
        ...
        public Task<T> SdkExecuteAsync<T>(IAlipayRequest<T> request) where T : AlipayResponse
        {
            try
            {
                logger?.LogDebug($"{DateTime.Now} 支付宝SdkExecuteAsync");
            ...
            {
                logger?.LogError($"{DateTime.Now} 支付宝SdkExecuteAsync报错", ex.Message);
                return null;
            }
        }
        ...
        private string Serialize(AlipayObject bizModel)
        {
            ...
            {
                logger?.LogError($"{DateTime.Now} 支付宝Serialize报错", ex.Message);
                return null;
            }
        }
