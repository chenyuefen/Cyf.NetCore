...
namespace TM.Core.Identity.JwtBearer.Internal
{
    /// <summary>
    /// Jwt构建器
    /// </summary>
    internal sealed class JsonWebTokenBuilder : IJsonWebTokenBuilder
    {
        ...
        public async Task<JsonWebToken> CreateAsync(IDictionary<string, string> payload, JwtOptions options)
        {
            ...
            var appId = payload["appId"] ?? Guid.NewGuid().ToString();
        ...
        public async Task<JsonWebToken> RefreshAsync(string refreshToken)
        {
            ...
            //解析出来payload原本就带有"aud": ["tmliveapp_client"]，而此时_options.Audience有值，CreateAsync创建的jwt会再叠加Audience
            //即新的jwt的payload会变成"aud": ["tmliveapp_client", "tmliveapp_client"]，新的token会变得无法使用
            if (_options.Audience == payload["aud"])
            {
                payload.Remove("aud");
            }
        ...
        public async Task<Dictionary<string, string>> GetPayloadByRefreshTokenAsync(string refreshToken)
        {
            if (string.IsNullOrWhiteSpace(refreshToken))
                throw new ArgumentNullException(nameof(refreshToken));
