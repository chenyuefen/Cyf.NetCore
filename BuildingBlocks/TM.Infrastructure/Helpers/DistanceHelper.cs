...
namespace TM.Infrastructure.Helpers
{
	...
	public class DistanceHelper
	{
		...
		/// 格式：纬度,经度
		/// </summary>
		/// <param name="d1">第一个点的经纬度</param>
		/// <param name="d2">第二个点的经纬度</param>
		/// <returns></returns>
		public static int Distance(string d1, string d2)
		{
			if (string.IsNullOrEmpty(d1) || string.IsNullOrEmpty(d2))
				return 0;
			var ds1 = d1.Split(',');
			var ds2 = d2.Split(',');
			if (ds1.Length != 2 || ds2.Length != 2)
				return 0;

			if (double.TryParse(ds1[1], out var n1) && double.TryParse(ds1[0], out var e1) && double.TryParse(ds2[1], out var n2) && double.TryParse(ds2[0], out var e2))
			{
				var distance = Distance(n1, e1, n2, e2);
		...
		/// <param name="d1">第一个点的经纬度</param>
		/// <param name="d2">第二个点的经纬度</param>
		/// <param name="n">保留小数点后几位</param>
		/// <returns></returns>
		public static string DistanceString(string d1, string d2, int n = 1)
		{
			var distance = Distance(d1, d2);
			if (distance > 1000)
				return $"{Math.Round(((double)distance / 1000), n, MidpointRounding.AwayFromZero)}公里";
			else
				return $"{distance}米";
		}
